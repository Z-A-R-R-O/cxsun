...
import { createCode128BarcodeSvg } from "./export-sales-barcode"
import { getExportSalesPrintLinePlan } from "./export-sales-print-line-plan"
import type { ExportSalesEntry, ExportSalesEntryItem } from "./export-sales-client"
...
export type ExportSalesPrintCopy = "duplicate" | "original" | "triplicate"
export interface ExportSalesPrintAddressLabels {
...
export interface ExportSalesPrintPartyDetails {
  addressLine: string
  gstin: string
  locationLine: string
  stateCode: string
...
export function ExportSalesInvoiceDocument({
  addressLabels,
  billingParty,
  company,
  copy = "original",
  ...
  readonly addressLabels?: ExportSalesPrintAddressLabels
  readonly billingParty?: ExportSalesPrintPartyDetails | null
  readonly company?: CompanyRecord | null
  readonly copy?: ExportSalesPrintCopy
  readonly customTerms?: string | null
  readonly documentTitle?: string
  readonly letterheadSettings?: Partial<LetterheadSettings>
  readonly record: ExportSalesEntry
  ...
  readonly shippingParty?: ExportSalesPrintPartyDetails | null
  ...
  const itemLinePlan = getExportSalesPrintLinePlan(record.items)
  ...
  const termsLines = ExportSalesPrintTerms(customTerms || record.terms)
  const hasIrn = Boolean(exportSalesDocumentValue(record, "irn"))
  const eInvoiceQrValue = useMemo(() => buildExportSalesEinvoiceQrPayload(record, company ?? null, totals), [company, record, totals])
  ...
  return (
    <MainPrintTemplate>
      <div className="grid grid-cols-[1fr_auto_1fr] p-px text-[9px]">
        ...
        <span className="text-right">{ExportSalesPrintCopyLabel(copy)}</span>
      </div>
      ...
      <table className={`${tableClass} border-t-0`} data-print-template={itemLinePlan.requiresTwoPageTemplate ? "two-page-required" : "single-page"}>
        ...
        <tbody>
          ...
          {itemLinePlan.rows.map((row) => row.kind === "item"
            ? <ExportSalesPrintItemRow key={`item-${row.index}`} columns={itemColumns} index={row.index} isCgstSgst={isCgstSgst} item={row.item} />
            : <BlankExportSalesPrintItemRow key={`blank-${row.index}`} columns={itemColumns} />)}
          <tr>
            {itemColumns.map((column, index) => (
...
function ExportSalesPrintItemRow({ columns, index, isCgstSgst, item }: { columns: ReturnType<typeof printItemColumns>; index: number; isCgstSgst: boolean; item: ExportSalesEntryItem }) {
...
function BlankExportSalesPrintItemRow({ columns }: { columns: ReturnType<typeof printItemColumns> }) {
...
function PartyAddressBlock({
  ...
  details?: ExportSalesPrintPartyDetails | null
  gstin?: string | null
  label: string
  partyName: string
...
function IrnDetailsBlock({ record }: { record: ExportSalesEntry }) {
  const ewayBillNo = exportSalesDocumentValue(record, "eway_bill_no")
  return (
    <div className="grid gap-1">
      <div className="grid grid-cols-[34px_1fr] gap-1">
        <span className="font-bold">IRN :</span>
        <span className="break-all font-bold leading-tight">{exportSalesDocumentValue(record, "irn") || record.uuid}</span>
      </div>
      <div className="grid gap-y-0.5">
        <InlinePrintPairRow
          leftLabel="Ack No.:"
          leftValue={exportSalesDocumentValue(record, "ack_no")}
          rightLabel="Ack Date:"
          rightValue={formatDate(exportSalesDocumentValue(record, "ack_date"))}
        />
        <InlinePrintPairRow
          leftLabel="E-Way Bill No.:"
          leftValue={ewayBillNo}
          rightLabel="Date:"
          rightValue={formatDate(exportSalesDocumentValue(record, "eway_bill_date"))}
        />
      </div>
...
function calculatePrintTotals(record: ExportSalesEntry) {
  return {
...
function itemTax(item: ExportSalesEntryItem) {
...
function sumQty(items: readonly ExportSalesEntryItem[]) {
...
function ExportSalesPrintTerms(value?: string | null) {
...
function ExportSalesPrintCopyLabel(copy: ExportSalesPrintCopy) {
  if (copy === "duplicate") return "Duplicate"
  if (copy === "triplicate") return "Office Copy"
  return "Original"
}

function exportSalesDocumentValue(record: ExportSalesEntry, key: string) {
