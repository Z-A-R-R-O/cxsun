...
type PaymentToolId = "downloadPdf" | "email" | "assign" | "attachments" | "tags" | "whatsapp"
type EntryListViewMode = "day" | "month"
type MonthlyPaymentSummary = { month: string; entryCount: number; amount: number; tdsAmount: number; discountAmount: number; roundOff: number; netAmount: number; allocatedAmount: number; unallocatedAmount: number }
    ...
    return (
      <PaymentShowPage
        ...
        onCorrection={async (entryValue) => {
          ...
          toast.success("Correction draft created", { description: correction?.payment_no ?? entryValue.payment_no })
          await refresh()
        ...
        onReversal={async (entryValue) => {
          ...
          toast.success("Reversal voucher created", { description: reversal?.payment_no ?? entryValue.payment_no })
          await refresh()
    ...
    <MasterListPageFrame
      ...
      action={
        <div className="flex items-center gap-2">
          ...
          <Button onClick={openNewEntry} type="button" className="h-9 rounded-md"><Plus className="size-4" />New Payment</Button>
        </div>
      }
    >
      <MasterListToolbarCard
          ...
          <table className="w-full min-w-[1040px] border-collapse text-sm">
            <thead className="bg-muted/55">
              <tr>
                ...
                <ListHeader className="text-center">Payments</ListHeader>
...
function PaymentShowPage({ entry, isWorking, onBack, onComment, onCorrection, onDestroy, onDownloadPdf, onEdit, onNew, onNext, onPrevious, onRestore, onReversal, onTool, session }: {
  entry: PaymentEntry
  isWorking: boolean
  onBack(): void
  onComment(entry: PaymentEntry, body: string): Promise<void>
  onCorrection(entry: PaymentEntry): Promise<void>
  onDestroy(): void
  onDownloadPdf(entry: PaymentEntry): Promise<void>
  onEdit(): void
  onNew(): void
  onNext?(): void
  onPrevious?(): void
  onRestore(): void
  onReversal(entry: PaymentEntry): Promise<void>
                ...
                <button disabled={isWorking} onClick={() => {
                  if (tool.id === "downloadPdf") {
                    void onDownloadPdf(entry)
                      .then(() => recordToolActivity(`Downloaded PDF ${entry.payment_no}`))
...
function PaymentUpsertPage({ entry, isSaving, onBack, onSubmit, session }: {
  ...
  const tabs: AnimatedTab[] = [
    { value: "details", label: "Details", content: <PaymentDetailsTab contacts={supplierContacts} form={draft} ledgers={ledgers} onCreateContact={setContactCreateInitialName} session={session} setForm={setDraft} /> },
...
function PaymentDetailsTab({ contacts, form, ledgers, onCreateContact, session, setForm }: { contacts: PaymentLookupOption[]; form: PaymentEntryInput; ledgers: AccountLedger[]; onCreateContact(name: string): void; session: AuthSession; setForm: Dispatch<SetStateAction<PaymentEntryInput>> }) {
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      ...
      <div className="space-y-5">
        ...
        <div className="grid gap-2">
          ...
          <Select value={form.payment_mode ?? "cash"} onValueChange={(value) => setForm((current) => ({ ...current, bank_account_id: null, ledger_id: null, ledger_name: null, payment_mode: value }))}>
        ...
        <LedgerAutocompleteLookup
          ...
          label={needsBank ? "Paid from bank ledger *" : "Cash ledger *"}
...
function openPurchaseBillOptions(entries: PurchaseEntry[], allocations: PaymentAllocation[]): OpenDocumentOption[] {
  ...
  return entries
    ...
    .map((entry) => {
      const allocated = (allocatedByDocument.get(String(entry.id)) ?? 0) + (allocatedByDocument.get(entry.uuid) ?? 0) + (allocatedByDocument.get(entry.entry_no) ?? 0)
      const balance = Math.max(0, Number(entry.balance_amount || 0) - allocated)
      return { balance, date: entry.entry_date, documentNo: entry.entry_no, id: String(entry.id), label: `${entry.entry_no} ${entry.supplier_name}`, partyName: entry.supplier_name, total: Number(entry.grand_total || 0) }
    })
...
function summarizePaymentsByMonth(entries: PaymentEntry[]): MonthlyPaymentSummary[] {
  const grouped = new Map<string, MonthlyPaymentSummary>()
  for (const entry of entries) {
    const month = monthKey(entry.payment_date)
...
function totalMonthlyPayments(rows: MonthlyPaymentSummary[]): MonthlyPaymentSummary {
  return rows.reduce(
    (total, row) => ({
      month: "total",
