...
import {
  ...
  emptyPurchaseReceiptEntry,
  emptyPurchaseReceiptItem,
  ...
  type PurchaseReceiptLookupOption,
  type PurchaseReceiptEntry,
  type PurchaseReceiptEntryInput,
  type PurchaseReceiptEntryItem,
} from "./purchase-receipt-client"
import { PurchaseReceiptEntryDocument, type PurchaseReceiptPrintCopy, type PurchaseReceiptPrintPartyDetails } from "./purchase-receipt-print-page"

type PurchaseReceiptView = { mode: "list" } | { mode: "show"; entry: PurchaseReceiptEntry } | { mode: "upsert"; entry: PurchaseReceiptEntry | null }
type PurchaseReceiptColumnId = "entry" | "date" | "supplier" | "updated"
type PurchaseReceiptEntryToolId = "email" | "assign" | "attachments" | "tags" | "whatsapp"
type PurchaseReceiptAddressLabels = {
...
const purchasePrintCopyOptions: readonly { label: string; value: PurchaseReceiptPrintCopy }[] = [
  { label: "Original", value: "original" },
  { label: "Duplicate", value: "duplicate" },
  { label: "Office Copy", value: "triplicate" },
]
const defaultPurchaseReceiptColumnVisibility: Record<PurchaseReceiptColumnId, boolean> = {
  supplier: true,
  date: true,
  entry: true,
  updated: false,
}
const purchaseColumnCatalog: Array<{ id: PurchaseReceiptColumnId; label: string }> = [
...
export function PurchaseReceiptPage({ session }: { session: AuthSession }) {
  const queryClient = useQueryClient()
  const [view, setView] = useState<PurchaseReceiptView>({ mode: "list" })
  ...
  const destroyMutation = useMutation({ mutationFn: (entry: PurchaseReceiptEntry) => destroyPurchaseReceiptEntry(session, entry) })
  ...
  useEffect(() => {
    if (entriesQuery.error) toast.error("Purchase receipt load failed", { description: entriesQuery.error instanceof Error ? entriesQuery.error.message : "Unable to load Purchase receipt entries." })
  }, [entriesQuery.error])
  ...
  async function save(input: PurchaseReceiptEntryInput, printAfterSave = false) {
    const entry = await upsertMutation.mutateAsync(input)
    toast.success(input.uuid ? "Purchase receipt entry updated" : "Purchase receipt entry created", { description: entry.entry_no })
    await refresh()
    setView({ mode: "show", entry })
    if (printAfterSave) window.setTimeout(() => window.print(), 300)
  }

  async function destroy(entry: PurchaseReceiptEntry) {
    await destroyMutation.mutateAsync(entry)
    toast.error("Purchase receipt entry suspended", { description: entry.entry_no })
    await refresh()
  }

  async function restore(entry: PurchaseReceiptEntry) {
    await restoreMutation.mutateAsync(entry)
    toast.success("Purchase receipt entry restored", { description: entry.entry_no })
    await refresh()
  }

  if (view.mode === "upsert") {
    return <PurchaseReceiptUpsertPage entry={view.entry} isSaving={upsertMutation.isPending} session={session} onBack={() => setView(view.entry ? { mode: "show", entry: view.entry } : { mode: "list" })} onSubmit={save} />
  }

  if (view.mode === "show") {
    const entry = entries.find((entry) => entry.uuid === view.entry.uuid) ?? view.entry
    return (
      <PurchaseReceiptShowPage
        entry={entry}
        ...
        onTool={async (entry, tool) => {
          ...
          toast.success(`${tool} queued`, { description: "The activity was recorded for this Purchase receipt entry." })
          await refresh()
          setView({ mode: "show", entry: updated })
        }}
      />
  ...
  return (
    <MasterListPageFrame
      ...
      description="Create and review purchase receipt entries."
    ...
    >
      ...
      <MasterListPaginationCard
        ...
        singularLabel="Purchase Receipt"
...
function PurchaseReceiptShowPage({ entry, isWorking, onBack, onComment, onDestroy, onEdit, onRestore, onTool, session }: {
  entry: PurchaseReceiptEntry
  isWorking: boolean
  onBack(): void
  onComment(entry: PurchaseReceiptEntry, body: string): Promise<void>
  onDestroy(): void
  onEdit(): void
  onRestore(): void
  onTool(entry: PurchaseReceiptEntry, tool: string): Promise<void>
  session: AuthSession
}) {
  const [comment, setComment] = useState("")
  const [printCopies, setPrintCopies] = useState<readonly PurchaseReceiptPrintCopy[]>(["original"])
  const [openTool, setOpenTool] = useState<PurchaseReceiptEntryToolId | null>(null)
  ...
  const addressLabels = usePurchaseReceiptAddressLabels(session)
  const companyQuery = useQuery({ queryKey: ["PurchaseReceipt-print-company", session.selectedTenant.slug], queryFn: () => listCompanies(session) })
  const contactsQuery = useQuery({ queryKey: ["PurchaseReceipt-print-contacts", session.selectedTenant.slug], queryFn: () => listPurchaseReceiptContactLookups(session) })
  ...
  const billingParty = buildPurchaseReceiptPrintPartyDetails(entry, selectedContact, entry.billing_address, addressLabels)
  const shippingParty = buildPurchaseReceiptPrintPartyDetails(entry, selectedContact, entry.shipping_address ?? entry.billing_address, addressLabels)
  ...
  function togglePrintCopy(copy: PurchaseReceiptPrintCopy) {
  ...
  function toggleEntryTool(tool: PurchaseReceiptEntryToolId) {
  ...
  const entryTools: Array<{ icon: typeof Mail; id: PurchaseReceiptEntryToolId; label: string }> = [
        ...
        <div className="grid gap-4 overflow-x-auto p-3 print:contents sm:p-4">
          {selectedPrintCopies.map((copy, index) => (
            <div key={copy} className={index === selectedPrintCopies.length - 1 ? "print:contents" : "print:break-after-page"}>
              <PurchaseReceiptEntryDocument addressLabels={addressLabels} billingParty={billingParty} company={printCompany} copy={copy} customTerms={customTerms} letterheadSettings={softwareSettings.letterheadSettings} record={entry} shippingParty={shippingParty} {...printItemSettings} />
            </div>
          ))}
        </div>
      </section>
...
function PurchaseReceiptUpsertPage({ entry, isSaving, session, onBack, onSubmit }: {
  entry: PurchaseReceiptEntry | null
  isSaving: boolean
  session: AuthSession
  onBack(): void
  onSubmit(input: PurchaseReceiptEntryInput, printAfterSave?: boolean): Promise<void>
}) {
  ...
  return (
    <MasterListPageFrame
      title={entry ? `Edit ${entry.entry_no}` : "New Purchase Receipt"}
    ...
    >
      ...
      {contactCreateInitialName !== null ? (
        <PurchaseReceiptContactCreateDialog
          ...
          onCreated={(contact) => {
            setDraft((current) => ({
              ...
              supplier_name: PurchaseReceiptLookupInputName(contact),
...
function PurchaseReceiptVoucherTabs({ contacts, form, hsnCodes, onContactsRefresh, onCreateContact, session, setForm, softwareSettings, units }: {
  ...
  const [itemDraft, setItemDraft] = useState<PurchaseReceiptEntryItem>(() => emptyPurchaseReceiptItem())
  const addressLabels = usePurchaseReceiptAddressLabels(session)

  function addItem() {
    if (!itemDraft.product_name.trim()) return
    setForm((current) => {
      const normalizedItem = normalizePurchaseReceiptItem(itemDraft, editingItemIndex ?? current.items.length)
    ...
    setItemDraft(emptyPurchaseReceiptItem())
  ...
  const tabs: AnimatedTab[] = [
    ...
    },
    {
      value: "address",
      label: "Address",
      content: <PurchaseReceiptAddressTab addressLabels={addressLabels} contacts={contacts} form={form} onContactsRefresh={onContactsRefresh} session={session} setForm={setForm} />,
    },
    {
      value: "terms",
      label: "Notes",
...
function PurchaseReceiptDetailsTab({ addItem, addressLabels, contacts, deleteItem, editItem, editingItemIndex, form, hsnCodes, itemDraft, onCreateContact, session, setEditingItemIndex, setForm, setItemDraft, softwareSettings, units }: {
  ...
  setForm(updater: (current: PurchaseReceiptEntryInput) => PurchaseReceiptEntryInput): void
  setItemDraft(value: PurchaseReceiptEntryItem | ((current: PurchaseReceiptEntryItem) => PurchaseReceiptEntryItem)): void
  softwareSettings: SoftwareSettingsState
  units: PurchaseReceiptLookupOption[]
}) {
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <div className="space-y-5">
          <MasterAutocompleteLookup
            ...
            selectedLabel={selectedPurchaseReceiptLookupInputLabel(contacts, form.supplier_id ?? null, form.supplier_name ?? "")}
            ...
            onPick={(option) => {
              ...
              setForm((current) => ({
                ...
                supplier_name: PurchaseReceiptLookupInputName(option),
    ...
    <div className="w-full overflow-x-auto rounded-md border border-border/70">
      <table className="w-full min-w-[760px] table-fixed border-collapse text-[10px] sm:text-[11px] xl:text-xs">
        ...
        <tbody>
          {items.length === 0 ? (
            <tr>
              <td colSpan={emptyColSpan} className="px-4 py-8 text-center text-sm text-muted-foreground">No purchase receipt items added.</td>
            </tr>
...
function PurchaseReceiptAddressTab({ addressLabels, contacts, form, onContactsRefresh, session, setForm }: {
  addressLabels: PurchaseReceiptAddressLabels
  contacts: PurchaseReceiptLookupOption[]
  form: PurchaseReceiptEntryInput
  onContactsRefresh(): void
  ...
  function pickAddress(kind: "billing" | "shipping", option: PurchaseReceiptLookupOption) {
  ...
  return (
    <>
      ...
      {createAddress && selectedContact ? (
        <PurchaseReceiptAddressCreateDialog
...
function PurchaseReceiptAddressCreateDialog({ addressLabels, contact, initialText, kind, onClose, onCreated, session }: {
  addressLabels: PurchaseReceiptAddressLabels
  contact: PurchaseReceiptLookupOption
  initialText: string
      ...
      <div className="w-[min(760px,calc(100vw-2rem))] overflow-hidden rounded-md border border-border/70 bg-card shadow-xl">
        <div className="flex items-center justify-between border-b border-border/70 px-5 py-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">{PurchaseReceiptLookupInputName(contact)}</p>
          </div>
...
function PurchaseReceiptTermsTab({ form, setForm }: {
  form: PurchaseReceiptEntryInput
  setForm(updater: (current: PurchaseReceiptEntryInput) => PurchaseReceiptEntryInput): void
}) {
  return (
...
function PurchaseReceiptContactCreateDialog({ contacts, initialName, onClose, onCreated, session }: {
  contacts: PurchaseReceiptLookupOption[]
  initialName: string
  isSaving: boolean
  onClose(): void
  onCreated(contact: PurchaseReceiptLookupOption): void
  session: AuthSession
}) {
  ...
  const contactTypesQuery = useQuery({ queryKey: ["PurchaseReceipt-contact-types", session.selectedTenant.slug], queryFn: () => listMasterDataRecords(session, "contactTypes") })
  const createMutation = useMutation({
    ...
    onSuccess: (contact) => {
      ...
      } as PurchaseReceiptLookupOption)
    },
  })
...
function buildContactAddressOptions(contact: PurchaseReceiptLookupOption | null, labels: PurchaseReceiptAddressLabels): PurchaseReceiptLookupOption[] {
  const options: PurchaseReceiptLookupOption[] = []
...
function contactAddresses(contact: PurchaseReceiptLookupOption | null): ContactAddress[] {
...
function contactLookupToInput(contact: PurchaseReceiptLookupOption): ContactInput {
...
function preferredContactAddress(contact: PurchaseReceiptLookupOption) {
...
function contactLookupGstin(contact: PurchaseReceiptLookupOption) {
  const record = contact.record as unknown as Partial<ContactRecord>
  return String(record.gstin ?? record.gstDetails?.find((item) => item.isDefault)?.gstin ?? record.gstDetails?.[0]?.gstin ?? "").trim().toUpperCase()
}

function addressText(address: Pick<ContactAddress, "addressLine1" | "addressLine2" | "cityId" | "districtId" | "stateId" | "countryId" | "pincodeId">, labels: PurchaseReceiptAddressLabels) {
  return [
...
function buildPurchaseReceiptPrintPartyDetails(entry: PurchaseReceiptEntry, contact: PurchaseReceiptLookupOption | null, savedAddress: string | null | undefined, labels: PurchaseReceiptAddressLabels): PurchaseReceiptPrintPartyDetails {
  const address = contact ? findMatchingContactAddress(contact, savedAddress, labels) : null
  const fallback = parseSavedPurchaseReceiptAddress(savedAddress)
...
function findMatchingContactAddress(contact: PurchaseReceiptLookupOption, savedAddress: string | null | undefined, labels: PurchaseReceiptAddressLabels) {
...
function parseSavedPurchaseReceiptAddress(address: string | null | undefined) {
...
function usePurchaseReceiptAddressLabels(session: AuthSession): PurchaseReceiptAddressLabels {
  const modules = ["addressTypes", "countries", "states", "districts", "cities", "pincodes"] as const
  const queries = modules.map((moduleKey) => useQuery({ queryKey: ["PurchaseReceipt-address-labels", session.selectedTenant.slug, moduleKey], queryFn: () => listMasterDataRecords(session, moduleKey) }))
  const maps = Object.fromEntries(modules.map((moduleKey, index) => [moduleKey, buildPurchaseReceiptLabelMap(queries[index].data ?? [])])) as Record<(typeof modules)[number], Map<string, string>>

  return {
    addressTypes: (value: unknown) => PurchaseReceiptLabelFrom(maps.addressTypes, value),
    cities: (value: unknown) => PurchaseReceiptLabelFrom(maps.cities, value),
    countries: (value: unknown) => PurchaseReceiptLabelFrom(maps.countries, value),
    districts: (value: unknown) => PurchaseReceiptLabelFrom(maps.districts, value),
    pincodes: (value: unknown) => PurchaseReceiptLabelFrom(maps.pincodes, value),
    states: (value: unknown) => PurchaseReceiptLabelFrom(maps.states, value),
    stateCodes: (value: unknown) => PurchaseReceiptCodeFrom(queries[2].data ?? [], value),
  }
}

function buildPurchaseReceiptLabelMap(records: MasterDataRecord[]) {
...
function PurchaseReceiptLabelFrom(map: ReadonlyMap<string, string>, value: unknown) {
  if (value === null || value === undefined || value === "") return ""
  return map.get(String(value)) ?? String(value)
}

function PurchaseReceiptCodeFrom(records: MasterDataRecord[], value: unknown) {
  if (value === null || value === undefined || value === "") return ""
...
function MasterAutocompleteLookup({
  ...
  className?: string
  createLabel?: string
  inputLabel?(option: PurchaseReceiptLookupOption): string
  inputRef?: Ref<HTMLInputElement>
  label: string
  onCreate?(query: string): void
  optionLabel?(option: PurchaseReceiptLookupOption): string
  onPick(option: PurchaseReceiptLookupOption): void
  onTextChange(value: string): void
  options: PurchaseReceiptLookupOption[]
  placeholder: string
  ...
  const getOptionLabel = optionLabel ?? ((option: PurchaseReceiptLookupOption) => option.label)
  ...
  function selectOption(option: PurchaseReceiptLookupOption) {
...
function selectedPurchaseReceiptLookupInputLabel(options: PurchaseReceiptLookupOption[], selectedId: string | null, fallback: string) {
  const selected = selectedId ? options.find((option) => option.id === selectedId) : null
  return selected ? PurchaseReceiptLookupInputName(selected) : fallback
}

function PurchaseReceiptLookupInputName(option: PurchaseReceiptLookupOption) {
...
function normalizePurchaseReceiptItem(item: PurchaseReceiptEntryItem, index: number): PurchaseReceiptEntryItem {
...
function searchPurchaseReceipt(entries: PurchaseReceiptEntry[], searchValue: string) {
...
function isActive(entry: PurchaseReceiptEntry) {
