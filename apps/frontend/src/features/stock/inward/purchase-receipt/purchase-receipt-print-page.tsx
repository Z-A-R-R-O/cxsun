...
import type { PurchaseReceiptEntry, PurchaseReceiptEntryItem } from "./purchase-receipt-client"
...
export type PurchaseReceiptPrintCopy = "duplicate" | "original" | "triplicate"
export interface PurchaseReceiptPrintAddressLabels {
...
export interface PurchaseReceiptPrintPartyDetails {
  addressLine: string
  gstin: string
  locationLine: string
  stateCode: string
...
export function PurchaseReceiptEntryDocument({
  addressLabels,
  billingParty,
  company,
  copy = "original",
  ...
  readonly addressLabels?: PurchaseReceiptPrintAddressLabels
  readonly billingParty?: PurchaseReceiptPrintPartyDetails | null
  readonly company?: CompanyRecord | null
  readonly copy?: PurchaseReceiptPrintCopy
  ...
  return (
    <MainPrintTemplate>
      <div className="grid grid-cols-[1fr_auto_1fr] p-px text-[9px]">
        ...
        <span className="text-right">{PurchaseReceiptPrintCopyLabel(copy)}</span>
      </div>
      ...
      <table className={`${tableClass} border-t-0`} data-print-template={itemLinePlan.requiresTwoPageTemplate ? "two-page-required" : "single-page"}>
        ...
        <tbody>
          ...
          {itemLinePlan.rows.map((row) => row.kind === "item"
            ? <PurchaseReceiptPrintItemRow key={`item-${row.index}`} columns={itemColumns} index={row.index} item={row.item} />
            : <BlankPurchaseReceiptPrintItemRow key={`blank-${row.index}`} columns={itemColumns} />)}
          <tr>
            {itemColumns.map((column, index) => (
          ...
          <tr>
            <td className={`${baseCell} border-r-0 p-0 leading-tight`} colSpan={itemColumns.length}>
              <div className="min-h-[96px] p-1.5 text-[8px] leading-[1.15]">
                <div>This purchase receipt records material receipt only. Pricing is retained for reference and does not post finance.</div>
...
function PurchaseReceiptPrintItemRow({ columns, index, item }: { columns: ReturnType<typeof printItemColumns>; index: number; item: PurchaseReceiptEntryItem }) {
...
function BlankPurchaseReceiptPrintItemRow({ columns }: { columns: ReturnType<typeof printItemColumns> }) {
...
function PartyAddressBlock({
  ...
  details?: PurchaseReceiptPrintPartyDetails | null
  gstin?: string | null
  label: string
  partyName: string
...
function companyHeaderDetails(company: CompanyRecord, labels?: PurchaseReceiptPrintAddressLabels) {
...
function PurchaseReceiptPrintCopyLabel(copy: PurchaseReceiptPrintCopy) {
