...
@Injectable()
export class AddressTypesCommonService {
  private readonly definition = addressTypesCommonDefinition

  constructor(
    @Inject(() => TenantContextService) private readonly tenantContext: TenantContextService,
    @Inject(AddressTypesCommonRepository) private readonly records: AddressTypesCommonRepository,
  ...
  async get(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) throw new NotFoundException(`AddressTypes common record was not found.`)
    return record
  }
  ...
  async destroy(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) return { ok: false, error: `AddressTypes common record was not found.` }
