...
import { BankNamesCommonRepository } from './repository.js'

@Injectable()
export class BankNamesCommonService {
  private readonly definition = bankNamesCommonDefinition

  constructor(
    @Inject(() => TenantContextService) private readonly tenantContext: TenantContextService,
    @Inject(BankNamesCommonRepository) private readonly records: BankNamesCommonRepository,
  ...
  async get(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) throw new NotFoundException(`BankNames common record was not found.`)
    return record
  }
  ...
  async destroy(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) return { ok: false, error: `BankNames common record was not found.` }
