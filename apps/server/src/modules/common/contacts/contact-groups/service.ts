...
@Injectable()
export class ContactGroupsCommonService {
  private readonly definition = contactGroupsCommonDefinition

  constructor(
    @Inject(() => TenantContextService) private readonly tenantContext: TenantContextService,
    @Inject(ContactGroupsCommonRepository) private readonly records: ContactGroupsCommonRepository,
  ...
  async get(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) throw new NotFoundException(`ContactGroups common record was not found.`)
    return record
  }
  ...
  async destroy(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) return { ok: false, error: `ContactGroups common record was not found.` }
