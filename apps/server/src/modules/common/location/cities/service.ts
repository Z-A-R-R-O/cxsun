...
import { CitiesCommonRepository } from './repository.js'

@Injectable()
export class CitiesCommonService {
  private readonly definition = citiesCommonDefinition

  constructor(
    @Inject(() => TenantContextService) private readonly tenantContext: TenantContextService,
    @Inject(CitiesCommonRepository) private readonly records: CitiesCommonRepository,
  ...
  async get(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) throw new NotFoundException(`Cities common record was not found.`)
    return record
  }
  ...
  async destroy(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) return { ok: false, error: `Cities common record was not found.` }
