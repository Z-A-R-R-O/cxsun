...
import { DestinationsCommonRepository } from './repository.js'

@Injectable()
export class DestinationsCommonService {
  private readonly definition = destinationsCommonDefinition

  constructor(
    @Inject(() => TenantContextService) private readonly tenantContext: TenantContextService,
    @Inject(DestinationsCommonRepository) private readonly records: DestinationsCommonRepository,
  ...
  async get(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) throw new NotFoundException(`Destinations common record was not found.`)
    return record
  }
  ...
  async destroy(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) return { ok: false, error: `Destinations common record was not found.` }
