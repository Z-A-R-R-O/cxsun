...
@Injectable()
export class PaymentTermsCommonService {
  private readonly definition = paymentTermsCommonDefinition

  constructor(
    @Inject(() => TenantContextService) private readonly tenantContext: TenantContextService,
    @Inject(PaymentTermsCommonRepository) private readonly records: PaymentTermsCommonRepository,
  ...
  async get(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) throw new NotFoundException(`PaymentTerms common record was not found.`)
    return record
  }
  ...
  async destroy(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) return { ok: false, error: `PaymentTerms common record was not found.` }
