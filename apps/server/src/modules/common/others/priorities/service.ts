...
import { PrioritiesCommonRepository } from './repository.js'

@Injectable()
export class PrioritiesCommonService {
  private readonly definition = prioritiesCommonDefinition

  constructor(
    @Inject(() => TenantContextService) private readonly tenantContext: TenantContextService,
    @Inject(PrioritiesCommonRepository) private readonly records: PrioritiesCommonRepository,
  ...
  async get(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) throw new NotFoundException(`Priorities common record was not found.`)
    return record
  }
  ...
  async destroy(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) return { ok: false, error: `Priorities common record was not found.` }
