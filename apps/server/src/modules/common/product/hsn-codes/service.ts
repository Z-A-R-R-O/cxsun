...
@Injectable()
export class HsnCodesCommonService {
  ...
  async get(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) throw new NotFoundException(`HsnCodes common record was not found.`)
    return record
  }
  ...
  async destroy(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) return { ok: false, error: `HsnCodes common record was not found.` }
