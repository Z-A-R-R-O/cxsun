...
@Injectable()
export class ProductCategoriesCommonService {
  private readonly definition = productCategoriesCommonDefinition

  constructor(
    @Inject(() => TenantContextService) private readonly tenantContext: TenantContextService,
    @Inject(ProductCategoriesCommonRepository) private readonly records: ProductCategoriesCommonRepository,
  ...
  async get(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) throw new NotFoundException(`ProductCategories common record was not found.`)
    return record
  }
  ...
  async destroy(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) return { ok: false, error: `ProductCategories common record was not found.` }
