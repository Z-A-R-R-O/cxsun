...
import { ProductGroupsCommonRepository } from './repository.js'

@Injectable()
export class ProductGroupsCommonService {
  private readonly definition = productGroupsCommonDefinition

  constructor(
    @Inject(() => TenantContextService) private readonly tenantContext: TenantContextService,
    @Inject(ProductGroupsCommonRepository) private readonly records: ProductGroupsCommonRepository,
  ...
  async get(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) throw new NotFoundException(`ProductGroups common record was not found.`)
    return record
  }
  ...
  async destroy(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) return { ok: false, error: `ProductGroups common record was not found.` }
