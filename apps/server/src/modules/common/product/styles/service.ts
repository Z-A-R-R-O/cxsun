...
import { StylesCommonRepository } from './repository.js'

@Injectable()
export class StylesCommonService {
  private readonly definition = stylesCommonDefinition

  constructor(
    @Inject(() => TenantContextService) private readonly tenantContext: TenantContextService,
    @Inject(StylesCommonRepository) private readonly records: StylesCommonRepository,
  ...
  async get(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) throw new NotFoundException(`Styles common record was not found.`)
    return record
  }
  ...
  async destroy(headers: TenantRequestHeaders, idOrUuid: string) {
    ...
    if (!record) return { ok: false, error: `Styles common record was not found.` }
