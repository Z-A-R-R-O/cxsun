...
export async function migrateExportSalesEntryTables(database: TenantDatabase) {
  await sql.raw(`
    CREATE TABLE IF NOT EXISTS export_sales_entries (
      ...
      INDEX idx_export_sales_entries_tenant_invoice (tenant_id, invoice_date, id),
      INDEX idx_export_sales_entries_company_year (company_id, accounting_year_id)
    )
  `).execute(database)
  ...
  await sql.raw(`
    CREATE TABLE IF NOT EXISTS export_sales_entry_items (
      id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
      uuid CHAR(8) NULL,
      export_sales_entry_id INT NOT NULL,
      ...
      created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      INDEX idx_export_sales_entry_items_parent (export_sales_entry_id, sort_order)
    )
  `).execute(database)

  await sql.raw(`
    CREATE TABLE IF NOT EXISTS export_sales_entry_comments (
      id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
      uuid CHAR(8) NULL,
      export_sales_entry_id INT NOT NULL,
      ...
      body TEXT NOT NULL,
      created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      INDEX idx_export_sales_entry_comments_parent (export_sales_entry_id, id)
    )
  `).execute(database)

  await sql.raw(`
    CREATE TABLE IF NOT EXISTS export_sales_entry_activities (
      id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
      uuid CHAR(8) NULL,
      export_sales_entry_id INT NOT NULL,
      ...
      created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      INDEX idx_export_sales_entry_activities_parent (export_sales_entry_id, id)
    )
  `).execute(database)

  await sql.raw(`
    UPDATE export_sales_entry_items item
    JOIN export_sales_entries entry ON entry.id = item.export_sales_entry_id
    SET
  ...
  await sql.raw(`
    UPDATE export_sales_entries entry
    JOIN (
      SELECT
        export_sales_entry_id,
      ...
      FROM export_sales_entry_items
      GROUP BY export_sales_entry_id
    ) totals ON totals.export_sales_entry_id = entry.id
    SET
