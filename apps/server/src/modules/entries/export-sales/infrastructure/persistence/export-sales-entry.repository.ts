...
import type { ExportSalesEntry, ExportSalesEntryItem } from '../../domain/entities/export-sales-entry.entity.js'

type DynamicDatabase = Record<string, Record<string, unknown>>

export interface ExportSalesEntryItemInput {
  id?: number
...
export interface ExportSalesEntryInput {
  id?: number
  uuid?: string
  company_id?: number
  ...
  terms?: string | null
  is_active?: boolean
  items?: ExportSalesEntryItemInput[]
}

@Injectable()
export class ExportSalesEntryRepository {
  ...
  async insert(context: TenantRuntimeContext, input: ExportSalesEntryInput) {
    const normalized = await this.normalize(context, input)
    const result = await this.database(context)
      .insertInto('export_sales_entries')
    ...
    await this.addActivityById(context, id, 'created', 'Export sales entry created')
    return this.find(context, String(id))
  }

  async update(context: TenantRuntimeContext, idOrUuid: string, input: ExportSalesEntryInput) {
    ...
    await this.database(context)
      .updateTable('export_sales_entries')
    ...
    await this.addActivityById(context, existing.id, 'updated', 'Export sales entry updated')
  ...
  async softDelete(context: TenantRuntimeContext, idOrUuid: string) {
    ...
    await this.addActivityById(context, existing.id, 'deleted', 'Export sales entry suspended')
    return this.find(context, String(existing.id))
  }
  ...
  async restore(context: TenantRuntimeContext, idOrUuid: string) {
    ...
    await this.addActivityById(context, existing.id, 'restored', 'Export sales entry restored')
    return this.find(context, String(existing.id))
  }
  ...
  async addComment(context: TenantRuntimeContext, idOrUuid: string, body: string) {
    ...
    await this.database(context)
      .insertInto('export_sales_entry_comments')
      .values({ uuid: this.nextUuid(), export_sales_entry_id: existing.id, author_email: context.user.email, body })
      .execute()
  ...
  private async normalize(context: TenantRuntimeContext, input: ExportSalesEntryInput, existingId?: number) {
  ...
  private async replaceItems(context: TenantRuntimeContext, ExportSalesEntryId: number, items: NormalizedSalesItem[]) {
    await this.database(context).deleteFrom('export_sales_entry_items').where('export_sales_entry_id', '=', ExportSalesEntryId).execute()
    await this.database(context)
      .insertInto('export_sales_entry_items')
      .values(items.map((item, index) => ({ ...item, uuid: this.nextUuid(), export_sales_entry_id: ExportSalesEntryId, sort_order: index + 1 })))
      .execute()
  }

  private async toEntry(context: TenantRuntimeContext, row: Record<string, unknown>): Promise<ExportSalesEntry> {
    const id = Number(row.id)
    const [items, comments, activities] = await Promise.all([
      this.database(context).selectFrom('export_sales_entry_items').selectAll().where('export_sales_entry_id', '=', id).orderBy('sort_order', 'asc').execute(),
      this.database(context).selectFrom('export_sales_entry_comments').selectAll().where('export_sales_entry_id', '=', id).orderBy('id', 'desc').execute(),
      this.database(context).selectFrom('export_sales_entry_activities').selectAll().where('export_sales_entry_id', '=', id).orderBy('id', 'desc').execute(),
    ])
    ...
    return {
      ...
      comments: comments.map((comment) => ({
        ...
        export_sales_entry_id: Number(comment.export_sales_entry_id),
      ...
      activities: activities.map((activity) => ({
        ...
        export_sales_entry_id: Number(activity.export_sales_entry_id),
  ...
  private async addActivityById(context: TenantRuntimeContext, ExportSalesEntryId: number, activityType: string, message: string) {
    await this.database(context)
      .insertInto('export_sales_entry_activities')
      .values({
        export_sales_entry_id: ExportSalesEntryId,
        uuid: this.nextUuid(),
  ...
  private async nextInvoiceNo(context: TenantRuntimeContext, companyId: number, accountingYearId: number) {
    for (let attempt = 0; attempt < 20; attempt += 1) {
      const documentNumber = await this.documentNumbers.consumeNext(context, 'exportSales', {
        accountingYearId: String(accountingYearId),
        companyId: String(companyId),
      })

      if (!documentNumber) {
        throw new BadRequestException('Invoice number is required when automatic export sales numbering is disabled.')
      }
  ...
  private async resolveInvoiceNo(context: TenantRuntimeContext, invoiceNo: string | undefined, companyId: number, accountingYearId: number, existingId?: number) {
    ...
    const preview = await this.documentNumbers.previewNext(context, 'exportSales', {
      accountingYearId: String(accountingYearId),
      companyId: String(companyId),
    })
    ...
    await this.documentNumbers.advancePast(context, 'exportSales', {
      accountingYearId: String(accountingYearId),
      companyId: String(companyId),
    }, trimmedInvoiceNo)
...
type NormalizedSalesItem = Omit<ExportSalesEntryItem, 'id' | 'uuid' | 'export_sales_entry_id' | 'sort_order'>

function normalizeItem(input: ExportSalesEntryItemInput, isCgstSgst: boolean): NormalizedSalesItem {
...
function defaultItem(): ExportSalesEntryItemInput {
  return { product_name: 'Item', quantity: 1, rate: 0, tax_rate: 0 }
}

function toItem(row: Record<string, unknown>): ExportSalesEntryItem {
  return {
    id: Number(row.id),
    uuid: String(row.uuid),
    export_sales_entry_id: Number(row.export_sales_entry_id),
