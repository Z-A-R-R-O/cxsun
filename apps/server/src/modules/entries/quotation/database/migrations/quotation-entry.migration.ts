...
export async function migrateQuotationEntryTables(database: TenantDatabase) {
  await sql.raw(`
    CREATE TABLE IF NOT EXISTS quotation_entries (
      ...
      INDEX idx_quotation_entries_tenant_invoice (tenant_id, invoice_date, id),
      INDEX idx_quotation_entries_company_year (company_id, accounting_year_id)
    )
  `).execute(database)

  await sql.raw(`ALTER TABLE quotation_entries ADD COLUMN IF NOT EXISTS generated_sales_invoice_uuid VARCHAR(80) NULL AFTER terms`).execute(database)
  await sql.raw(`ALTER TABLE quotation_entries ADD COLUMN IF NOT EXISTS generated_sales_invoice_no VARCHAR(80) NULL AFTER generated_sales_invoice_uuid`).execute(database)

  await sql.raw(`
    CREATE TABLE IF NOT EXISTS quotation_entry_items (
      id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
      uuid CHAR(8) NULL,
      quotation_entry_id INT NOT NULL,
      ...
      created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      INDEX idx_quotation_entry_items_parent (quotation_entry_id, sort_order)
    )
  `).execute(database)

  await sql.raw(`
    CREATE TABLE IF NOT EXISTS quotation_entry_comments (
      id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
      uuid CHAR(8) NULL,
      quotation_entry_id INT NOT NULL,
      ...
      body TEXT NOT NULL,
      created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      INDEX idx_quotation_entry_comments_parent (quotation_entry_id, id)
    )
  `).execute(database)

  await sql.raw(`
    CREATE TABLE IF NOT EXISTS quotation_entry_activities (
      id INT NOT NULL AUTO_INCREMENT PRIMARY KEY,
      uuid CHAR(8) NULL,
      quotation_entry_id INT NOT NULL,
      ...
      created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
      INDEX idx_quotation_entry_activities_parent (quotation_entry_id, id)
    )
  `).execute(database)

  await sql.raw(`
    UPDATE quotation_entry_items item
    JOIN quotation_entries entry ON entry.id = item.quotation_entry_id
    SET
  ...
  await sql.raw(`
    UPDATE quotation_entries entry
    JOIN (
      SELECT
        quotation_entry_id,
      ...
      FROM quotation_entry_items
      GROUP BY quotation_entry_id
    ) totals ON totals.quotation_entry_id = entry.id
    SET
