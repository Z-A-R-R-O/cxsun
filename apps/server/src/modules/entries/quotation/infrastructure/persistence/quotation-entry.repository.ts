...
import type { QuotationEntry, QuotationEntryItem } from '../../domain/entities/quotation-entry.entity.js'

type DynamicDatabase = Record<string, Record<string, unknown>>

export interface QuotationEntryItemInput {
  id?: number
...
export interface QuotationEntryInput {
  id?: number
  uuid?: string
  company_id?: number
  ...
  is_active?: boolean
  items?: QuotationEntryItemInput[]
}

@Injectable()
export class QuotationEntryRepository {
  ...
  async insert(context: TenantRuntimeContext, input: QuotationEntryInput) {
    const normalized = await this.normalize(context, input)
    const result = await this.database(context)
      .insertInto('quotation_entries')
    ...
    await this.addActivityById(context, id, 'created', 'Quotation entry created')
    return this.find(context, String(id))
  }

  async update(context: TenantRuntimeContext, idOrUuid: string, input: QuotationEntryInput) {
    const existing = await this.find(context, idOrUuid)
    if (!existing) return null
    if (existing.status === 'invoiced' || existing.generated_sales_invoice_uuid) {
      throw new BadRequestException(`Quotation ${existing.invoice_no} is already invoiced. Suspend the generated sales invoice before editing this quotation.`)
    }
    const normalized = await this.normalize(context, input, existing.id)

    await this.database(context)
      .updateTable('quotation_entries')
    ...
    await this.addActivityById(context, existing.id, 'updated', 'Quotation entry updated')
  ...
  async softDelete(context: TenantRuntimeContext, idOrUuid: string) {
    ...
    await this.addActivityById(context, existing.id, 'deleted', 'Quotation entry suspended')
    return this.find(context, String(existing.id))
  }
  ...
  async restore(context: TenantRuntimeContext, idOrUuid: string) {
    ...
    await this.addActivityById(context, existing.id, 'restored', 'Quotation entry restored')
    return this.find(context, String(existing.id))
  }
  ...
  async addComment(context: TenantRuntimeContext, idOrUuid: string, body: string) {
    ...
    await this.database(context)
      .insertInto('quotation_entry_comments')
      .values({ uuid: this.nextUuid(), quotation_entry_id: existing.id, author_email: context.user.email, body })
      .execute()
  ...
  async addActivityByUuid(context: TenantRuntimeContext, idOrUuid: string, activityType: string, message: string) {
    const entry = await this.addActivity(context, idOrUuid, activityType, message)
    if (!entry) throw new BadRequestException('Quotation entry was not found.')
    return entry
  }

  async markInvoicedBySalesInvoice(context: TenantRuntimeContext, quotationUuids: string[], invoice: { invoice_no: string; uuid: string }) {
    const entries = await this.findManyByUuids(context, quotationUuids)
    if (entries.length !== quotationUuids.length) throw new BadRequestException('One or more source quotations were not found.')
    const conflict = entries.find((entry) => entry.generated_sales_invoice_uuid && entry.generated_sales_invoice_uuid !== invoice.uuid)
    if (conflict) {
      throw new BadRequestException(`Quotation ${conflict.invoice_no} is already linked to sales invoice ${conflict.generated_sales_invoice_no ?? conflict.generated_sales_invoice_uuid}.`)
    }

    await this.database(context)
      .updateTable('quotation_entries')
      .set({
        generated_sales_invoice_no: invoice.invoice_no,
        generated_sales_invoice_uuid: invoice.uuid,
        status: 'invoiced',
        updated_at: new Date(),
      })
      .where('tenant_id', '=', context.tenant.id)
      .where('uuid', 'in', quotationUuids)
      .execute()

    await Promise.all(entries.map((entry) => this.addActivityById(context, entry.id, 'invoiced', `Marked invoiced by sales invoice ${invoice.invoice_no}.`)))
  }

  async releaseBySalesInvoice(context: TenantRuntimeContext, invoice: { invoice_no: string; uuid: string }) {
    const entries = await this.findManyByGeneratedInvoice(context, invoice.uuid)
    ...
    await this.database(context)
      ...
      .where('tenant_id', '=', context.tenant.id)
      .where('generated_sales_invoice_uuid', '=', invoice.uuid)
      .execute()

    await Promise.all(entries.map((entry) => this.addActivityById(context, entry.id, 'invoice-released', `Released after sales invoice ${invoice.invoice_no} was suspended.`)))
    return entries
  }

  private async normalize(context: TenantRuntimeContext, input: QuotationEntryInput, existingId?: number) {
    ...
    return {
      entry: {
        ...
        generated_sales_invoice_uuid: emptyAsNull(input.generated_sales_invoice_uuid),
        generated_sales_invoice_no: emptyAsNull(input.generated_sales_invoice_no),
  ...
  private async replaceItems(context: TenantRuntimeContext, quotationEntryId: number, items: NormalizedQuotationItem[]) {
    await this.database(context).deleteFrom('quotation_entry_items').where('quotation_entry_id', '=', quotationEntryId).execute()
    await this.database(context)
      .insertInto('quotation_entry_items')
      .values(items.map((item, index) => ({ ...item, uuid: this.nextUuid(), quotation_entry_id: quotationEntryId, sort_order: index + 1 })))
      .execute()
  }

  private async toEntry(context: TenantRuntimeContext, row: Record<string, unknown>): Promise<QuotationEntry> {
    const id = Number(row.id)
    const [items, comments, activities] = await Promise.all([
      this.database(context).selectFrom('quotation_entry_items').selectAll().where('quotation_entry_id', '=', id).orderBy('sort_order', 'asc').execute(),
      this.database(context).selectFrom('quotation_entry_comments').selectAll().where('quotation_entry_id', '=', id).orderBy('id', 'desc').execute(),
      this.database(context).selectFrom('quotation_entry_activities').selectAll().where('quotation_entry_id', '=', id).orderBy('id', 'desc').execute(),
    ])
    ...
    return {
      ...
      generated_sales_invoice_uuid: stringOrNull(row.generated_sales_invoice_uuid),
      generated_sales_invoice_no: stringOrNull(row.generated_sales_invoice_no),
      ...
      comments: comments.map((comment) => ({
        ...
        quotation_entry_id: Number(comment.quotation_entry_id),
      ...
      activities: activities.map((activity) => ({
        ...
        quotation_entry_id: Number(activity.quotation_entry_id),
  ...
  private async addActivityById(context: TenantRuntimeContext, quotationEntryId: number, activityType: string, message: string) {
    await this.database(context)
      .insertInto('quotation_entry_activities')
      .values({
        quotation_entry_id: quotationEntryId,
        uuid: this.nextUuid(),
  ...
  private async findManyByUuids(context: TenantRuntimeContext, uuids: string[]) {
  ...
  private async findManyByGeneratedInvoice(context: TenantRuntimeContext, invoiceUuid: string) {
    const rows = await this.database(context)
      .selectFrom('quotation_entries')
      .selectAll()
      .where('tenant_id', '=', context.tenant.id)
      .where('generated_sales_invoice_uuid', '=', invoiceUuid)
      .execute()
    return Promise.all(rows.map((row) => this.toEntry(context, row)))
  }
  ...
  private async nextInvoiceNo(context: TenantRuntimeContext, companyId: number, accountingYearId: number) {
    const rows = await this.database(context)
      ...
      .where('accounting_year_id', '=', accountingYearId)
      .execute()
    const maxSerial = rows.reduce((max, row) => Math.max(max, parseQuotationSerial(String(row.invoice_no))), 0)
    return `QUO-${String(maxSerial + 1).padStart(4, '0')}`
  }
  ...
  private async resolveInvoiceNo(context: TenantRuntimeContext, invoiceNo: string | undefined, companyId: number, accountingYearId: number, existingId?: number) {
    ...
    if (await this.invoiceNoExists(context, trimmedInvoiceNo, companyId, accountingYearId, existingId)) {
      throw new BadRequestException(`Quotation number ${trimmedInvoiceNo} already exists.`)
    }
...
type NormalizedQuotationItem = Omit<QuotationEntryItem, 'id' | 'uuid' | 'quotation_entry_id' | 'sort_order'>

function normalizeItem(input: QuotationEntryItemInput, isCgstSgst: boolean): NormalizedQuotationItem {
...
function defaultItem(): QuotationEntryItemInput {
  return { product_name: 'Item', quantity: 1, rate: 0, tax_rate: 0 }
}

function toItem(row: Record<string, unknown>): QuotationEntryItem {
  return {
    id: Number(row.id),
    uuid: String(row.uuid),
    quotation_entry_id: Number(row.quotation_entry_id),
...
function parseQuotationSerial(value: string) {
  const match = /^QUO-(\d+)$/i.exec(value.trim())
