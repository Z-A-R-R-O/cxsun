...
import type { PurchaseReceipt, PurchaseReceiptItem } from '../../domain/entities/purchase-receipt.entity.js'
...
export class PurchaseReceiptRepository {
  ...
  async insert(context: TenantRuntimeContext, input: PurchaseReceiptInput) {
    const normalized = await this.normalize(context, input)
    const result = await this.database(context)
      .insertInto('stock_purchase_receipts')
    ...
    await this.addActivityById(context, id, 'created', 'purchase receipt created')
    return this.find(context, String(id))
  }

  async update(context: TenantRuntimeContext, idOrUuid: string, input: PurchaseReceiptInput) {
    const existing = await this.find(context, idOrUuid)
    if (!existing) return null
    const normalized = await this.normalize(context, input)

    await this.database(context)
      .updateTable('stock_purchase_receipts')
    ...
    await this.addActivityById(context, existing.id, 'updated', 'purchase receipt updated')
  ...
  async softDelete(context: TenantRuntimeContext, idOrUuid: string) {
    ...
    await this.addActivityById(context, existing.id, 'deleted', 'purchase receipt suspended')
    return this.find(context, String(existing.id))
  }
  ...
  async restore(context: TenantRuntimeContext, idOrUuid: string) {
    ...
    await this.addActivityById(context, existing.id, 'restored', 'purchase receipt restored')
    return this.find(context, String(existing.id))
  }
  ...
  async addComment(context: TenantRuntimeContext, idOrUuid: string, body: string) {
    ...
    await this.database(context)
      .insertInto('stock_purchase_receipt_comments')
      .values({ uuid: this.nextUuid(), purchase_receipt_id: existing.id, author_email: context.user.email, body })
      .execute()
  ...
  private async normalize(context: TenantRuntimeContext, input: PurchaseReceiptInput) {
  ...
  private async replaceItems(context: TenantRuntimeContext, PurchaseReceiptId: number, items: NormalizedPurchaseItem[]) {
    await this.database(context).deleteFrom('stock_purchase_receipt_items').where('purchase_receipt_id', '=', PurchaseReceiptId).execute()
    await this.database(context)
      .insertInto('stock_purchase_receipt_items')
      .values(items.map((item, index) => ({ ...item, uuid: this.nextUuid(), purchase_receipt_id: PurchaseReceiptId, sort_order: index + 1 })))
      .execute()
  }

  private async toEntry(context: TenantRuntimeContext, row: Record<string, unknown>): Promise<PurchaseReceipt> {
    const id = Number(row.id)
    const [items, comments, activities] = await Promise.all([
      this.database(context).selectFrom('stock_purchase_receipt_items').selectAll().where('purchase_receipt_id', '=', id).orderBy('sort_order', 'asc').execute(),
      this.database(context).selectFrom('stock_purchase_receipt_comments').selectAll().where('purchase_receipt_id', '=', id).orderBy('id', 'desc').execute(),
      this.database(context).selectFrom('stock_purchase_receipt_activities').selectAll().where('purchase_receipt_id', '=', id).orderBy('id', 'desc').execute(),
    ])
    ...
    return {
      ...
      comments: comments.map((comment) => ({
        ...
        purchase_receipt_id: Number(comment.purchase_receipt_id),
      ...
      activities: activities.map((activity) => ({
        ...
        purchase_receipt_id: Number(activity.purchase_receipt_id),
  ...
  private async addActivityById(context: TenantRuntimeContext, PurchaseReceiptId: number, activityType: string, message: string) {
    await this.database(context)
      .insertInto('stock_purchase_receipt_activities')
      .values({
  ...
  private async nextEntryNo(context: TenantRuntimeContext, companyId: number, accountingYearId: number) {
    const documentNumber = await this.documentNumbers.consumeNext(context, 'purchaseReceipt', {
      accountingYearId: String(accountingYearId),
      companyId: String(companyId),
    })

    if (!documentNumber) {
      throw new BadRequestException('Entry number is required when automatic purchase receipt numbering is disabled.')
    }
  ...
  private async resolveEntryNo(context: TenantRuntimeContext, entryNo: string | undefined, companyId: number, accountingYearId: number) {
    ...
    const preview = await this.documentNumbers.previewNext(context, 'purchaseReceipt', {
      accountingYearId: String(accountingYearId),
      companyId: String(companyId),
    })
...
type NormalizedPurchaseItem = Omit<PurchaseReceiptItem, 'id' | 'uuid' | 'purchase_receipt_id' | 'sort_order'>

function normalizeItem(input: PurchaseReceiptItemInput): NormalizedPurchaseItem {
...
function defaultItem(): PurchaseReceiptItemInput {
  return { product_name: 'Item', quantity: 1, rate: 0, tax_rate: 0 }
}

function toItem(row: Record<string, unknown>): PurchaseReceiptItem {
  return {
    id: Number(row.id),
    uuid: String(row.uuid),
