...
import type { DeliveryNote, DeliveryNoteItem } from '../../domain/entities/delivery-note.entity.js'
...
export class DeliveryNoteRepository {
  ...
  async insert(context: TenantRuntimeContext, input: DeliveryNoteInput) {
    const normalized = await this.normalize(context, input)
    const result = await this.database(context)
      .insertInto('stock_delivery_notes')
    ...
    await this.addActivityById(context, id, 'created', 'delivery note created')
  ...
  async update(context: TenantRuntimeContext, idOrUuid: string, input: DeliveryNoteInput) {
    ...
    await this.addActivityById(context, existing.id, 'updated', 'delivery note updated')
  ...
  async softDelete(context: TenantRuntimeContext, idOrUuid: string) {
    ...
    await this.addActivityById(context, existing.id, 'deleted', 'delivery note suspended')
    return this.find(context, String(existing.id))
  }
  ...
  async restore(context: TenantRuntimeContext, idOrUuid: string) {
    ...
    await this.addActivityById(context, existing.id, 'restored', 'delivery note restored')
    return this.find(context, String(existing.id))
  }
  ...
  async addComment(context: TenantRuntimeContext, idOrUuid: string, body: string) {
    ...
    await this.database(context)
      .insertInto('stock_delivery_note_comments')
      .values({ uuid: this.nextUuid(), delivery_note_id: existing.id, author_email: context.user.email, body })
      .execute()
  ...
  private async replaceItems(context: TenantRuntimeContext, DeliveryNoteId: number, items: NormalizedPurchaseItem[]) {
    await this.database(context).deleteFrom('stock_delivery_note_items').where('delivery_note_id', '=', DeliveryNoteId).execute()
    await this.database(context)
      .insertInto('stock_delivery_note_items')
      .values(items.map((item, index) => ({ ...item, uuid: this.nextUuid(), delivery_note_id: DeliveryNoteId, sort_order: index + 1 })))
      .execute()
  }

  private async toEntry(context: TenantRuntimeContext, row: Record<string, unknown>): Promise<DeliveryNote> {
    const id = Number(row.id)
    const [items, comments, activities] = await Promise.all([
      this.database(context).selectFrom('stock_delivery_note_items').selectAll().where('delivery_note_id', '=', id).orderBy('sort_order', 'asc').execute(),
      this.database(context).selectFrom('stock_delivery_note_comments').selectAll().where('delivery_note_id', '=', id).orderBy('id', 'desc').execute(),
      this.database(context).selectFrom('stock_delivery_note_activities').selectAll().where('delivery_note_id', '=', id).orderBy('id', 'desc').execute(),
    ])
    ...
    return {
      ...
      comments: comments.map((comment) => ({
        ...
        delivery_note_id: Number(comment.delivery_note_id),
      ...
      activities: activities.map((activity) => ({
        ...
        delivery_note_id: Number(activity.delivery_note_id),
  ...
  private async addActivityById(context: TenantRuntimeContext, DeliveryNoteId: number, activityType: string, message: string) {
    await this.database(context)
      .insertInto('stock_delivery_note_activities')
      .values({
  ...
  private async resolveEntryNo(context: TenantRuntimeContext, entryNo: string | undefined, companyId: number, accountingYearId: number) {
    ...
    const preview = await this.documentNumbers.previewNext(context, 'deliveryNote', {
      accountingYearId: String(accountingYearId),
      companyId: String(companyId),
    })
...
type NormalizedPurchaseItem = Omit<DeliveryNoteItem, 'id' | 'uuid' | 'delivery_note_id' | 'sort_order'>

function normalizeItem(input: DeliveryNoteItemInput): NormalizedPurchaseItem {
...
function defaultItem(): DeliveryNoteItemInput {
  return { product_name: 'Item', quantity: 1, rate: 0, tax_rate: 0 }
}

function toItem(row: Record<string, unknown>): DeliveryNoteItem {
  return {
    id: Number(row.id),
    uuid: String(row.uuid),
    delivery_note_id: Number(row.delivery_note_id),
