...
| 2026-05-16 | Use `id INT AUTO_INCREMENT PRIMARY KEY` plus `uuid CHAR(8) NOT NULL UNIQUE` on application tables. | Numeric IDs stay fast and stable for internal joins, while short public UUIDs hide sequence IDs from APIs and UI. Move public UUIDs to 16 characters later when scale requires it. |
...
- Keep `id` as the internal primary key and `uuid` as the public identifier on application-owned tables. Current public UUIDs are 8 characters; plan a 16-character migration later when growth requires it.
